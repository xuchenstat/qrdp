...
qrjointgcs <- function(x, y, group.ix, nsamp = 1e3, thin = 10, cens = NULL,
                    wt = NULL, incr = 0.01, par = "prior", nknots = 6,
                    hyper = list(sig = c(.1,.1), lam = c(6,4), kap = c(0.1,0.1,1)),
                    shrink = FALSE, prox.range = c(.2,.95), acpt.target = 0.15, acpt.gtarget = 0.15,
                    ref.size = 3, ref.gsize = 3, blocking = "std5", temp = 1, expo = 2,
  ...
  group.count <- count(group.ix)
  if(length(which(group.count$freq < 2))==0){
    group <- group.count
  } else {
    group <- group.count[-which(group.count$freq < 2),]
  }
  ngroups <- nrow(group)
  groups.size <- group$freq
  groups <- vector('list', ngroups)
  for(g in 1:ngroups){
    groups[[g]] <- which(group.ix==group$x[g])
  }
  max.gsize <- max(group$freq)
  ...
  niter <- nsamp * thin
  dimpars <- c(n, p, L, mid - 1, nknots, ngrid, ncol(a.kap), niter, thin, ngroups, nsamp)
  ...
  imcmc.par <- c(nblocks, ref.size, verbose, max(10, niter/1e4), ref.gsize, max.gsize, rep(0, nblocks), rep(0, ngroups))
  dmcmc.par <- c(temp, 0.999, rep(acpt.target, nblocks), 2.38 / sqrt(blocks.size), rep(acpt.gtarget, ngroups), rep(2.38, ngroups))
  ...
  logitr <- qlogis(0.5) #qlogis(c(0.1,seq(0.1,0.9,length.out = 9)))
  theta <- rep(0, ngroups)
  phi <- rep(1, ngroups)
  tm.c <- system.time(oo <- .C("BJQRgcs", par = as.double(parmg), lgtr = as.double(logitr),  x = as.double(x), y = as.double(y), cens = as.integer(cens), wt = as.double(wt),
                               shrink = as.integer(shrink), hyper = as.double(hyper.reduced), dim = as.integer(dimpars), gridmats = as.double(gridmats),
                               tau.g = as.double(tau.g), muV = as.double(blocks.mu), SV = as.double(blocks.S), theta = as.double(theta), phi = as.double(phi), 
                               blocks = as.integer(blocks.ix), blocks.size = as.integer(blocks.size), groupsVar = as.integer(unlist(groups) - 1), groups_size = as.integer(groups.size), dmcmcpar = as.double(dmcmc.par), 
                               imcmcpar = as.integer(imcmc.par), parmgsamp = double(nsamp * length(parmg)), 
                               acptsamp = double(nsamp * nblocks), lpsamp = double(nsamp), lgtrsamp = double(nsamp * ngroups), acptlgtrsamp = double(nsamp*ngroups),
...
coef.qrjointgcs <- function(object, burn.perc = 0.5, nmc = 200, plot = FALSE, show.intercept = TRUE, reduce = TRUE, ...){
  nsamp <- object$dim[11]                      # number of iterations retains in sample
  ngroups <- object$dim[10]
