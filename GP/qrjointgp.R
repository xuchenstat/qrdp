...
qrjointgp <- function(x, y, location = NULL, distance = NULL, nsamp = 1e3, thin = 10, cens = NULL,
                       ...
                       ref.size = 3, ref.hpsize = 3, blocking = "std5", blockinghp = '1', temp = 1, expo = 2,
                       blocks.mu, blocks.S, blocks.muhp, blocks.Shp, fix.nu = FALSE, fbase = c("t", "logistic", "unif"), verbose = TRUE){
  ...
  niter <- nsamp * thin
  nparhp <- 2
  dimpars <- c(n, p, L, mid - 1, nknots, ngrid, ncol(a.kap), niter, thin, nparhp, nsamp)
  ...
  if(blockinghp == "1"){                                                # ONE BLOCK
    blockshp <- list(rep(TRUE, nparhp))                                        # Update all simultaneously
  } else if(blockinghp == "2"){                                        # TWO BLOCKS
    blockshp <- replicate(nparhp, rep(FALSE, nparhp), simplify = FALSE)
    for(i in 1:nparhp) blockshp[[i]][i] <- TRUE    
  } else{
    blockshp <- replicate(nparhp+1, rep(FALSE, nparhp), simplify = FALSE)
    for(i in 1:nparhp) blockshp[[i]][i] <- TRUE
    blockshp[[nparhp+1]][1:nparhp] <- TRUE
  } 
  
  nblockshp <- length(blockshp)

  blockshp.ix <- c(unlist(lapply(blockshp, which))) - 1                       # Index of locations of updates
  blockshp.size <- sapply(blockshp, sum)  
  if(missing(blocks.muhp)) blocks.muhp <- rep(0, sum(blockshp.size))
  if(missing(blocks.Shp)){
    blocks.Shp <- lapply(blockshp.size, function(q) diag(1, q))
    blocks.Shp <- unlist(blocks.Shp)
  }
  
  imcmc.par <- c(nblocks, ref.size, verbose, max(10, niter/1e4), nblockshp, ref.hpsize, rep(0, nblocks), rep(0, nblockshp))
  dmcmc.par <- c(temp, 0.999, rep(acpt.target, nblocks), 2.38 / sqrt(blocks.size), rep(acpt.hptarget, nblockshp), 2.38 / sqrt(blockshp.size))
  ...
  Mhyper <- c(log(mean(distance)), qlogis(2/3, 0, 1))
  
  tm.c <- system.time(oo <- .C("BJQRgp", par = as.double(parmg), Mhyper = as.double(Mhyper), distance = as.double(distance), x = as.double(x), y = as.double(y), cens = as.integer(cens), wt = as.double(wt),
                               shrink = as.integer(shrink), hyper = as.double(hyper.reduced), dim = as.integer(dimpars), gridmats = as.double(gridmats),
                               tau.g = as.double(tau.g), muV = as.double(blocks.mu), SV = as.double(blocks.S), muhpVar = as.double(blocks.muhp), ShpVar = as.double(blocks.Shp), 
                               blocks = as.integer(blocks.ix), blocks.size = as.integer(blocks.size), blockshp = as.integer(blockshp.ix), blockshp.size = as.integer(blockshp.size), dmcmcpar = as.double(dmcmc.par), 
                               imcmcpar = as.integer(imcmc.par), parmgsamp = double(nsamp * nparmg), 
                               acptsamp = double(nsamp * nblocks), lpsamp = double(nsamp), hpsamp = double(nsamp * nparhp), acpthpsamp = double(nsamp*nblockshp),
                               lmglhsamp = double(nsamp), fbase.choice = as.integer(fbase.choice)))
...
coef.qrjointgp <- function(object, burn.perc = 0.5, nmc = 200, plot = FALSE, show.intercept = TRUE, reduce = TRUE, ...){
