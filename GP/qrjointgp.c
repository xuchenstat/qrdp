...
void adMCMCgp(int niter, int thin, int n, double *distance, int nparmg, double *parmg, double *Mhyper, int nparhp, int nblocks, int **blocks, int *blocks_size, double **mu, double ***S, int nblockshp, int **blockshp, int *blockshp_size, double **muhp, double ***Shp, double *acpt_target, double *acpt_hptarget,
			double decay, int refresh, int refreshhp, double *lm, double *lmhp, double temp,  
			int *refresh_counter, int *refreshhp_counter, int verbose, int ticker,
			double *parmgsamp, double *acptsamp, double *lpsamp, double *hpsamp, double *acpthpsamp, double *lmglhsamp);
...
double kappaFn_inv(double s);
double kappaFn(double z);
double lgauCopDen(int p, double *x, double **Cinvchol);
void corInvChol(int p, double *distance, double *Mhyper, double **Cinvchol);
double matern2(double x, double phi, double kappa);
double lprior(int p, double *Mhyper, double **Cinvchol);
//global integers
...
double *llvec, *pgvec, *rpvec, *lp, *zcop, *zprod, *cte, *diagcop, **C, **Rcop;
double phi, kappa; 
int *pivot, *rank;
...
void BJQRgp(double *parmg, double *Mhyper, double *distance, double *xVar, double *yVar, int *status, double *weights, int *toShrink, double *hyper, int *dim, double *gridpars,
          double *tauG, double *muVar, double *SVar, double *muhpVar, double *ShpVar, int *blocksVar, int *blocks_size, int *blockshpVar, int *blockshp_size, double *dmcmcpar, int *imcmcpar,
          double *parmgsamp, double *acptsamp, double *lpsamp, double *hpsamp, double *acpthpsamp, double *lmglhsamp, int *distribution){
    ...
    dist = distribution[0];
    int niter = dim[reach++], thin = dim[reach++], nparhp = dim[reach++], nparmg = (m+1)*(p+1) + 2;
    taugrid = tauG;
    ...
    int b, h;
    int nblocks = imcmcpar[0], refresh = imcmcpar[1], verbose = imcmcpar[2], ticker = imcmcpar[3], nblockshp = imcmcpar[4], refreshhp = imcmcpar[5];
    int *refresh_counter = imcmcpar + 6, *refreshhp_counter = imcmcpar + 6 + nblocks;
    
    double temp = dmcmcpar[0], decay = dmcmcpar[1];
    double *acpt_target = dmcmcpar + 2, *lm = acpt_target + nblocks, *acpt_hptarget = lm + nblocks, *lmhp = acpt_hptarget + nblockshp;
    ...
    double **muhp = (double **)R_alloc(nblockshp, sizeof(double *));
    double ***Shp = (double ***)R_alloc(nblockshp, sizeof(double **));
    int **blockshp = (int **)R_alloc(nblockshp, sizeof(int *));

    int muhp_point = 0, Shp_point = 0, blockshp_point = 0;
    for(h = 0; h < nblockshp; h++){
        muhp[h] = muhpVar + muhp_point; muhp_point += blockshp_size[h];        
        Shp[h] = (double **)R_alloc(blockshp_size[h], sizeof(double *));
        for(i = 0; i < blockshp_size[h]; i++){
            Shp[h][i] = ShpVar + Shp_point;
            Shp_point += blockshp_size[h];
        }
        blockshp[h] = blockshpVar + blockshp_point; blockshp_point += blockshp_size[h];
    }

    //Call to adaptive MCMC code
    adMCMCgp(niter, thin, n, distance, nparmg, parmg, Mhyper, nparhp, nblocks, blocks, blocks_size, mu, S, nblockshp, blockshp, blockshp_size, muhp, Shp, acpt_target, acpt_hptarget, decay, refresh, refreshhp, lm, lmhp, temp, refresh_counter, refreshhp_counter, verbose, ticker, parmgsamp, acptsamp, lpsamp, hpsamp, acpthpsamp, lmglhsamp);
...
void adMCMCgp(int niter, int thin, int n, double *distance, int nparmg, double *parmg, double *Mhyper, int nparhp, int nblocks, int **blocks, int *blocks_size, double **mu, double ***S, int nblockshp, int **blockshp, int *blockshp_size, double **muhp, double ***Shp, double *acpt_target, double *acpt_hptarget,
	...
	for(b = 0; b < nblocks; b++){
        ...
        double ***Rhp = (double ***)R_alloc(nblockshp, sizeof(double **));
	int **blockshp_pivot = (int **)R_alloc(nblockshp, sizeof(double *)), *blockshp_rank = ivect(nblockshp);
	double *blockshp_d = vect(nparhp);
	for(h = 0; h < nblockshp; h++){
		Rhp[h] = mymatrix(blockshp_size[h], blockshp_size[h]);
		blockshp_pivot[h] = ivect(blockshp_size[h]);
                //Rprintmat("Shp", "%10g", Shp[h], blockshp_size[h], blockshp_size[h], 1);
		chol(Rhp[h], blockshp_size[h], 1.0e-8, blockshp_pivot[h], blockshp_rank + h, blockshp_size[h], blockshp_d, Shp[h], 0, 0, 1.0e-10);
	}
	...
	double *frac = vect(nblocks);
        // copula part
        int *hp_size = ivect(nblockshp);
      	double *acpt_hp = vect(nblockshp), **hpbar = (double **)R_alloc(nblockshp, sizeof(double *)); 
        double *alphahp = vect(nblockshp), *frachp = vect(nblockshp);
	...
	for(b = 0; b < nblocks; b++){
		...
		hp_size[h] = 0;
		acpt_hp[h] = 0.0;
		hpbar[h] = vect(blockshp_size[h]);
                for(i = 0; i < blockshp_size[h]; i++) hpbar[h][i] = 0.0;				
		frachp[h] = sqrt(1.0 / ((double)refreshhp_counter[h] + 1.0));
	}


	double lp_diff, lcop_diff, lmglh, lmglhnew, lcop, lcopnew, lpr, lprnew, lcpost, lcpostnew;
	double **parmgstore = mymatrix(2, nparmg), *par_incr = vect(nparmg), *zsamp = vect(nparmg), **hpstore = mymatrix(2, nparhp), *parhp_incr = vect(nparhp), *zsamphp = vect(nparhp);
        double **U = mymatrix(2, n), ***Cinvchol = array3d(2, n, n);
	int ipar = 0, iparnew = 1, iparhp = 0, iparhpnew = 1;

	for(i = 0; i < nparmg; i++) parmgstore[ipar][i] = parmg[i];
	for(i = 0; i < nparhp; i++) hpstore[iparhp][i] = Mhyper[i];

	logpostFn(parmgstore[ipar], temp, 0, llvec, pgvec, y, cens, lp, rpvec);
	lmglh = lp[0];
        for(i = 0; i < n; i++) U[ipar][i] = rpvec[i];
        corInvChol(n, distance, hpstore[iparhp], Cinvchol[iparhp]);
        lcop = lgauCopDen(n, U[ipar], Cinvchol[iparhp]);
        lpr = lprior(n, hpstore[iparhp], Cinvchol[iparhp]);
        lcpost = lcop + lpr;

        if(verbose) Rprintf("Initial lp = %g\n", lmglh + lcpost);
	int iter, store_lp = 0, store_parmg = 0, store_acpt = 0, store_lmglh = 0, store_hp = 0, store_acpthp = 0;
	double *alpha_run = vect(nblocks), *alphahp_run = vect(nblockshp), chs, lambda;
        int *run_counter = ivect(nblocks), *runhp_counter = ivect(nblockshp);
	for(b = 0; b < nblocks; b++) {alpha_run[b] = 0.0; run_counter[b] = 0;}
	for(h = 0; h < nblockshp; h++) {alphahp_run[h] = 0.0; runhp_counter[h] = 0;}
	...
	for(iter = 0; iter < niter; iter++){
		for(b = 0; b < nblocks; b++){
			...
			for(i = 0; i < blocks_size[b]; i++) parmgstore[iparnew][blocks[b][i]] += lambda * par_incr[i];
                        ...
                        lcopnew = lgauCopDen(n, U[iparnew], Cinvchol[iparhp]);
                        
			lp_diff = lmglhnew - lmglh + lcopnew - lcop;
			...
			run_counter[b]++;
                ...
                for(h = 0; h < nblockshp; h++){
                  hp_size[h]++;
		  for(i = 0; i < blockshp_size[h]; i++) zsamphp[i] = rnorm(0.0, 1.0);
		  triprod(Rhp[h], blockshp_size[h], blockshp_size[h], zsamphp, parhp_incr, 1);
		  for(i = 0; i < nparhp; i++) hpstore[iparhpnew][i] = hpstore[iparhp][i];
                  for(i = 0; i < blockshp_size[h]; i++) hpstore[iparhpnew][blockshp[h][i]] += lmhp[h] * parhp_incr[i];
                  corInvChol(n, distance, hpstore[iparhpnew], Cinvchol[iparhpnew]);
                  lcopnew = lgauCopDen(n, U[ipar], Cinvchol[iparhpnew]);
                  lprnew = lprior(n, hpstore[iparhpnew], Cinvchol[iparhpnew]);
                  lcpostnew = lcopnew + lprnew;
                  
                  //Rprintf("lgtr = %g", lgtrstore[iparrnew[g]][g]);                  
                  //Rprintf("lcopnew = %g", lcopnew[g]);
                  //Rprintf("lprnew = %g", lprnew[g]);
                  //Rprintf("\n");
                  lcop_diff = lcpostnew - lcpost;
                  alphahp[h] = exp(lcop_diff); if(alphahp[h] > 1.0) alphahp[h] = 1.0;
                  if(runif(0.0, 1.0) < alphahp[h]){      
				iparhp = iparhpnew;
				iparhpnew = !iparhp;
                                lcop = lcopnew;
                                lpr = lprnew;
                  ...
                  alphahp_run[h] = ((double)runhp_counter[h] * alphahp_run[h] + alphahp[h]) / ((double)(runhp_counter[h] + 1.0));
		  runhp_counter[h]++;
                 }

                
		if((iter + 1) % thin == 0){
			lpsamp[store_lp++] = lmglh + lcpost;
			...
			for(b = 0; b < nblocks; b++) acptsamp[store_acpt++] = alpha[b];
                        for(i = 0; i < nparhp; i++) hpsamp[store_hp++] = hpstore[iparhp][i];
                        for(h = 0; h < nblockshp; h++) acpthpsamp[store_acpthp++] = alphahp[h];                                
		}
		
		if(verbose){
			if(niter < ticker || (iter + 1) % (niter / ticker) == 0){
				Rprintf("iter = %d, lp = %g ", iter + 1, lmglh + lcpost);
		...
		for(h = 0; h < nblockshp; h++){
			chs = (double) hp_size[h]; if(chs < 1.0) chs = 1.0;
			acpt_hp[h] += (alphahp[h] - acpt_hp[h]) / chs;
			for(i = 0; i < blockshp_size[h]; i++)
				hpbar[h][i] += (hpstore[ipar][blockshp[h][i]] - hpbar[h][i]) / chs; 
			if(hp_size[h] == refreshhp * blockshp_size[h]){
				refreshhp_counter[h]++;
				frachp[h] = sqrt(1.0 / ((double)refreshhp_counter[h] + 1.0));
				for(i = 0; i < blockshp_size[h]; i++){
					for(j = 0; j < i; j++){
						Shp[h][i][j] = (1.0 - frachp[h]) * Shp[h][i][j] + frachp[h] * (hpbar[h][i] - muhp[h][i]) * (hpbar[h][j] - muhp[h][j]);
						Shp[h][j][i] = Shp[h][i][j];
					}
					Shp[h][i][i] = (1.0 - frachp[h]) * Shp[h][i][i] + frachp[h] * (hpbar[h][i] - muhp[h][i]) * (hpbar[h][i] - muhp[h][i]);
				}
				chol(Rhp[h], blockshp_size[h], 1.0e-8, blockshp_pivot[h], blockshp_rank + h, blockshp_size[h], blockshp_d, Shp[h], 0, 0, 1.0e-10);
				for(i = 0; i < blockshp_size[h]; i++){
                                  muhp[h][i] += frachp[h] * (hpbar[h][i] - muhp[h][i]);
                                  hpbar[h][i] = 0.0;
                                }
				lmhp[h] *= exp(frachp[h] * (acpt_hp[h] - acpt_hptarget[h]));
				acpt_hp[h] = 0.0;
				hp_size[h] = 0;
			}
		}
...
double lprior(int p, double *Mhyper, double **Cinvchol){
  ...
  result += Mhyper[0] - log(1.0 + exp(2.0*Mhyper[0])) + dlogis(Mhyper[1], 0.0, 1.0, 1);
  //result += Mhyper[0] - exp(Mhyper[0]) + dlogis(Mhyper[1], 0.0, 1.0, 1);
  return result;
}
	...
	if (uphi==0.0) return 1; 
		...
		if (kappa==0.5) 
			ans = exp(-uphi); 
                        ...
                        ans = cte[0] * R_pow(uphi, kappa) * bessel_k(uphi,kappa,1); 
		} 
	} 
	return ans; 
}
...
void corInvChol(int p, double *distance, double *Mhyper, double **Cinvchol){
  int i, j, counter = 0;
  phi = exp(Mhyper[0]); kappa = kappaFn(Mhyper[1]);
  cte[0] = R_pow(2.0, (-(kappa-1)))/gammafn(kappa); 
  for(i = 0; i < p; i++){
    for(j = i + 1; j < p; j++){
      C[i][j] = matern2(distance[counter++], phi, kappa);
      C[j][i] = C[i][j];
    }
  }
...
// Allows kappa to be sampled on full real and returned to a value in [0, 3]
double kappaFn(double z){
	return 3.0*plogis(z, 0.0, 1.0, 1, 0);
} 	

// Function that returns logit(s/3); Return kappa from a real value in [0, 3] back to full real
double kappaFn_inv(double s){
	return qlogis(s/3.0, 0.0, 1.0, 1, 0);
}
