...
void adMCMCar1(int niter, int thin, int n, int nparmg, double *parmg, double r, int nblocks, int **blocks, int *blocks_size, double **mu, double ***S, double theta, double phi, double *acpt_target, double acpt_rtarget, double decay, int refresh, int refreshr, double *lm, double lmr, double temp,  
			int *refresh_counter, int refreshr_counter, int verbose, int ticker, double *parmgsamp, double *acptsamp, double *lpsamp, double *rhosamp, double *acptrhosamp, double *lmglhsamp);
...
double lgauCopDen(int p, double *x, double rho);
double lprior(int p, double rho);
//global integers
...
double *llvec, *pgvec, *rpvec, *lp, *zcop, *shock;
...
void BJQRar1(double *parmg, double *rinit, double *xVar, double *yVar, int *status, double *weights, int *toShrink, double *hyper, int *dim, double *gridpars,
          double *tauG, double *muVar, double *SVar, double *thetaVar, double *phiVar, int *blocksVar, int *blocks_size, double *dmcmcpar, int *imcmcpar,
          double *parmgsamp, double *acptsamp, double *lpsamp, double *rhosamp, double *acptrhosamp, double *lmglhsamp, int *distribution){
    ...
    int b;
    int nblocks = imcmcpar[0], refresh = imcmcpar[1], verbose = imcmcpar[2], ticker = imcmcpar[3], refreshr = imcmcpar[4], refreshr_counter = imcmcpar[5];
    int *refresh_counter = imcmcpar + 6;
    
    double temp = dmcmcpar[0], decay = dmcmcpar[1], acpt_rtarget = dmcmcpar[2], lmr = dmcmcpar[3];
    ...
    double r = rinit[0], theta = thetaVar[0], phi = phiVar[0];

    //Call to adaptive MCMC code
    adMCMCar1(niter, thin, n, nparmg, parmg, r, nblocks, blocks, blocks_size, mu, S, theta, phi, acpt_target, acpt_rtarget, decay, refresh, refreshr, lm, lmr, temp,  
			refresh_counter, refreshr_counter, verbose, ticker, parmgsamp, acptsamp, lpsamp, rhosamp, acptrhosamp, lmglhsamp);
}
...
void adMCMCar1(int niter, int thin, int n, int nparmg, double *parmg, double r, int nblocks, int **blocks, int *blocks_size, double **mu, double ***S, double theta, double phi, double *acpt_target, double acpt_rtarget, double decay, int refresh, int refreshr, double *lm, double lmr, double temp,  
	...
	double *frac = vect(nblocks);
        // copula part
        int rho_size = 0;
      	double acpt_rho = 0.0, rhobar = 0.0, alpharho = 0.0, fracrho = 0.0;
	...
	double lp_diff, lcop_diff, lmglh, lmglhnew, lcop, lcopnew, lpr, lprnew, lcpost, lcpostnew, rho = r, rhonew;
	...
	lmglh = lp[0];
        ...
        lcop = lgauCopDen(n, U[ipar], rho);
        lpr = lprior(n, rho);
        lcpost = lcop + lpr;

        if(verbose) Rprintf("Initial lp = %g\n", lmglh + lcpost);
	int iter, store_lp = 0, store_parmg = 0, store_acpt = 0, store_lmglh = 0, store_rho = 0, store_acptrho = 0;
	double *alpha_run = vect(nblocks), alpharho_run = 0.0, chs, lambda;
        int *run_counter = ivect(nblocks), runrho_counter = 0;
	...
	for(iter = 0; iter < niter; iter++){
		for(b = 0; b < nblocks; b++){
			...
			for(i = 0; i < blocks_size[b]; i++) parmgstore[iparnew][blocks[b][i]] += lambda * par_incr[i];
                        ...
                        lcopnew = lgauCopDen(n, U[iparnew], rho);
		  ...
		  rhonew = rho + lmr*zsamp[0]*phi;
                  lcopnew = lgauCopDen(n, U[ipar], rhonew);
                  lprnew = lprior(n, rhonew);
                  ...
                  alpharho = exp(lcop_diff); if(alpharho > 1.0) alpharho = 1.0;
                  if(runif(0.0, 1.0) < alpharho){      
			        rho = rhonew;
                                lcop = lcopnew;
                                lpr = lprnew;
                                lcpost = lcpostnew;
		  }
                  alpharho_run = ((double)runrho_counter * alpharho_run + alpharho) / ((double)(runrho_counter + 1.0));
		  runrho_counter++;
		...
		if((iter + 1) % thin == 0){
			...
			for(b = 0; b < nblocks; b++) acptsamp[store_acpt++] = alpha[b];
                        rhosamp[store_rho++] = rho;
                        acptrhosamp[store_acptrho++] = alpharho;                                
		}
		...
		for(b = 0; b < nblocks; b++){
			...
			chs = (double) rho_size; if(chs < 1.0) chs = 1.0;
			acpt_rho += (alpharho - acpt_rho) / chs;
                        rhobar += (rho - rhobar) / chs; 
			if(rho_size == refreshr){
				refreshr_counter++;
				fracrho = sqrt(1.0 / ((double)refreshr_counter + 1.0));
                                phi = sqrt((1.0 - fracrho) * phi*phi + fracrho * (rhobar - theta) * (rhobar - theta));
                                theta += fracrho * (rhobar - theta);
                                rhobar = 0.0;
                                }
				lmr *= exp(fracrho * (acpt_rho - acpt_rtarget));
				acpt_rho = 0.0;
				rho_size = 0;
        }
	PutRNGstate();
...
double lprior(int p, double rho){
  int i;
  double ans, r = 2.0 * plogis(rho, 0.0, 1.0, 1, 0) - 1.0;
  ans = -(p - 1.0)*log(1.0 - r * r)/2.0 + dlogis(rho, 0.0, 1.0, 1);
  return ans;
}
...
double lgauCopDen(int p, double *x, double rho){
  int i;
  double ans = 0.0, r = 2.0 * plogis(rho, 0.0, 1.0, 1, 0) - 1.0;
  zcop[0] = qnorm(x[0], 0.0, 1.0, 1, 0);
  for(i = 1; i < p; i++) {
    zcop[i] = qnorm(x[i], 0.0, 1.0, 1, 0);
    shock[i-1] = zcop[i] - r * zcop[i-1];
  }
  for(i = 1; i < p; i++) ans += shock[i-1]*shock[i-1];
  ans += (1.0 - r * r)*zcop[0]*zcop[0];
  ans /= (1.0 - r * r);
  ans -= sumsquares(zcop, p);
  return (-ans/2.0);
}
