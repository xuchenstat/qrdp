...
qrjointar1 <- function(x, y, nsamp = 1e3, thin = 10, cens = NULL,
                      wt = NULL, incr = 0.01, par = "prior", nknots = 6,
                      hyper = list(sig = c(.1,.1), lam = c(6,4), kap = c(0.1,0.1,1)),
                      shrink = FALSE, prox.range = c(.2,.95), acpt.target = 0.15, acpt.rtarget = 0.15,
                      ref.size = 3, ref.rsize = 3, blocking = "std5", temp = 1, expo = 2,
  ...
  imcmc.par <- c(nblocks, ref.size, verbose, max(10, niter/1e4), ref.rsize, 0, rep(0, nblocks))
  dmcmc.par <- c(temp, 0.999, acpt.rtarget, 2.38, rep(acpt.target, nblocks), 2.38 / sqrt(blocks.size))
  ...
  theta <- 0
  phi <- 1
  tm.c <- system.time(oo <- .C("BJQRar1", par = as.double(parmg), rinit = as.double(rinit), x = as.double(x), y = as.double(y), cens = as.integer(cens), wt = as.double(wt),
                               shrink = as.integer(shrink), hyper = as.double(hyper.reduced), dim = as.integer(dimpars), gridmats = as.double(gridmats),
                               tau.g = as.double(tau.g), muV = as.double(blocks.mu), SV = as.double(blocks.S), thetaVar = as.double(theta), phiVar = as.double(phi), 
                               blocks = as.integer(blocks.ix), blocks.size = as.integer(blocks.size), dmcmcpar = as.double(dmcmc.par), 
                               imcmcpar = as.integer(imcmc.par), parmgsamp = double(nsamp * nparmg), 
                               acptsamp = double(nsamp * nblocks), lpsamp = double(nsamp), rhosamp = double(nsamp), acptrhosamp = double(nsamp),
...
coef.qrjointar1 <- function(object, burn.perc = 0.5, nmc = 200, plot = FALSE, show.intercept = TRUE, reduce = TRUE, ...){
